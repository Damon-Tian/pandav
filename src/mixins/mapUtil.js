...
export default {
  ...
  methods: {
    ...
    async setLayer(type, id, geoData) {
      ...
      if (type == 2) {
        ...
        const option = {
          ...
          lineWidth: geoData[0].lineType ? 1 : 4,
          arrow: false
        }
        //设置线条名称
        if (geoData[0].textName) {
    ...
    reset() {
      ...
      const currentItem = this.hrefs.find(
        (item) => item.id == this.$store.state.app.currentArea
      )
    ...
    flyToDetailZoom(center, feature) {
      ...
      feature &&
        this.$store.state.app.defaultComponent.handleClick(feature, [feature])
    }
  }
}
